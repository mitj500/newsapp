    ...
    async componentDidMount(){ 
     ...
     handleNextClick = async ()=>{
        ...
        if (this.state.page + 1 > Math.ceil(this.state.totalResults/20)){
            ...
            let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}dbe57b028aeb41e285a226a94865f7a7&page=${this.state.page + 1}&pageSize=20`;
            let data = await fetch(url);
            let parsedData = await data.json()
            console.log(parsedData);  
            this.setState({
